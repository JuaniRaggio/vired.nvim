...
describe("vired.watcher", function()
  ...
  describe("toggle", function()
    it("should enable watcher when disabled", function()
      assert.is_false(watcher.is_watching(test_bufnr))
      local result = watcher.toggle(test_bufnr, temp_dir)
      assert.is_true(result)
      assert.is_true(watcher.is_watching(test_bufnr))
    end)

    it("should disable watcher when enabled", function()
      watcher.start(test_bufnr, temp_dir)
      assert.is_true(watcher.is_watching(test_bufnr))
      local result = watcher.toggle(test_bufnr, temp_dir)
    ...
    it("should work even when config has watcher disabled", function()
      config.setup({ watcher = { enabled = false } })
      -- Toggle should bypass config and enable manually
      local result = watcher.toggle(test_bufnr, temp_dir)
      assert.is_true(result)
      assert.is_true(watcher.is_watching(test_bufnr))
    end)
